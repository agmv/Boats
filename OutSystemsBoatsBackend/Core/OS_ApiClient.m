...
NSString *const OS_ResponseObjectErrorKey = @"OS_ResponseObject";
...
static NSDictionary * OS___headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * OS___fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = OS___headerFieldsForResponse(response);
...
@interface OS_ApiClient ()

@property (nonatomic, strong) NSDictionary* HTTPResponseHeaders;

@end

@implementation OS_ApiClient

- (instancetype)init {
    NSString *baseUrl = [[OS_Configuration sharedConfig] host];
...
- (instancetype)initWithBaseURL:(NSURL *)url {
    ...
    if (self) {
        ...
        self.responseDeserializer = [[OS_ResponseDeserializer alloc] init];
        self.sanitizer = [[OS_Sanitizer alloc] init];
...
+(NSNumber*) queueRequest {
    ...
    OS_DebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        OS_DebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    } else {
        return NO;
...
- (void) configureCacheReachibility {
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        reachabilityStatus = status;
        OS_DebugLog(@"reachability changed to %@",AFStringFromNetworkReachabilityStatus(status));
        [OS_ApiClient setOfflineState:status == AFNetworkReachabilityStatusUnknown || status == AFNetworkReachabilityStatusNotReachable];
...
- (void) operationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        OS_DebugLogResponse(response, responseObject,request,error);
        strongSelf.HTTPResponseHeaders = OS___headerFieldsForResponse(response);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[OS_ResponseObjectErrorKey] = responseObject;
        }
...
- (void) downloadOperationWithCompletionBlock: (NSURLRequest *)request
    ...
    NSURLSessionDataTask* op = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString * filename = OS___fileNameForResponse(response);
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [OS_JSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [OS_JSONResponseSerializer serializer];
    } else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = OS_PercentEscapedStringFromString(safeString);
    ...
    if (offlineState) {
        OS_DebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        OS_DebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        OS_DebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [OS_ApiClient queueRequest];
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = OS_PercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, OS_PercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[OS_QueryParamCollection class]]){
            OS_QueryParamCollection * coll = (OS_QueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = OS_PercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = OS_PercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = OS_PercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (AFSecurityPolicy *) customSecurityPolicy {
    ...
    OS_Configuration *config = [self configuration];
...
- (OS_Configuration*) configuration {
    return [OS_Configuration sharedConfig];
}
